export default class Extension {
  ...
  private async sendMessage<T>(message: unknown): Promise<T> {
    return new Promise((resolve, rejects) => {
      if (chrome.runtime && this.isInstalled) {
        chrome.runtime.sendMessage("nmlkohaecenocojodbbbcigopcombbef", message, (response) => {
          resolve(response);
        });
      } else {
  ...
  async getUser(id: number): Promise<[NvAPIUser | null, NvAPIRelationships | null]> {
    const user = await this.sendMessage<NvAPIResponse<NvAPIUserResponse>>({
      type: "fetchApi",
      payload: {
  ...
  async getOwnUser(): Promise<[NvAPIOwnUser | null, boolean]> {
    const user = await this.sendMessage<NvAPIResponse<NvAPIOwnUserResponse>>({
      type: "fetchApi",
      payload: {
  ...
  async getVideoWatch(videoId: string, isLogin: boolean): Promise<WatchData | null> {
    ...
    const watch = await this.sendMessage<WatchAPIResponse<WatchData>>({
      type: "fetchApi",
      payload: {
        url: url.toString(),
        method: "GET",
  ...
  async sendVideoWatchEvent(trackingId: string): Promise<boolean> {
    ...
    const event = await this.sendMessage<NvAPIResponse<null>>({
      type: "fetchApi",
      payload: {
        url: url.toString(),
        method: "GET",
  ...
  async sendVideoSession(
    media: WatchMediaDelivery,
    protocol: "http" | "hls",
    maxVideoSrcId: string
    ...
    const sessionRes = await this.sendMessage<SessionAPIResponse>({
      type: "fetchApi",
      payload: {
  ...
  async getVideoComments(
    ...
    const comments = await this.sendMessage<NvCommentAPIResponse<NvCommentGetResponse>>({
      type: "fetchApi",
      payload: {
  ...
  async getVideoCommentThreadkey(threadId: number | string): Promise<string | null> {
    const threadKey = await this.sendMessage<NvAPIResponse<NvAPIThreadkey>>({
      type: "fetchApi",
      payload: {
  ...
  async getVideoCommentPostkey(threadId: number | string): Promise<string | null> {
    const postKey = await this.sendMessage<NvAPIResponse<NvAPIPostkey>>({
      type: "fetchApi",
      payload: {
  ...
  async postVideoComment(
    ...
    commands: string[],
    vposMs: number
  ): Promise<[number | null, string | null, string | null]> {
    ...
    const postCommentRes = await this.sendMessage<NvCommentAPIResponse<NvCommentPostResponse>>({
      type: "fetchApi",
      payload: {
